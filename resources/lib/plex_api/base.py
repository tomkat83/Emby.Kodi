...
class Base(object):
    ...
    def ratingtype(self):
        """
        Returns the ratingtype [str] first from 'audienceRatingImage' (audience)
        if that fails, from ratingImage (critic).
        Returns 'default' if both are not found
        """
        #catch-all
        rtype = 'default'
        #audience ratings
        ...
        rtypec = self.xml.get('ratingImage', '')
